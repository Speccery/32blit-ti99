...
/* EPEPEP commented out, implemented elsewhere
...
/*
  static int debug_start=1; // EPEP
  static FILE *debug_log = nullptr;
  if (debug_start == 1) { 
     debug_log = fopen("cpu9900log.txt", "wt");
     if (debug_log) fprintf(debug_log, "line:pc  :addr:data:st\n");
  }
  if (debug_start && debug_log) {
    fprintf(debug_log, "%4d:%04X:%04X:%04X:%04X\n", debug_start, PC, ep_last_write_address, ep_last_write_data, ST);
    debug_start++;
    if (debug_start == 100001) {
      debug_start = 0;
      fclose(debug_log);
    }
  }
